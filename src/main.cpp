...
uint256 hashGenesisBlock("0x3e8624cdd31c8ecf3e4589e8fd555d5b7d748716c102fee5dbe6d3d1a5bac6a4");
...
const string strMessageMagic = "catecoin Signed Message:\n";
...
bool CTransaction::AreInputsStandard(const MapPrevTx& mapInputs) const
{
    ...
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        ...
        // be quick, CTCause if there are any operations
...
static const int64 nTargetTimespan = 1 * 1 * 10 * 60; // catecoin: 10 minutes
static const int64 nTargetSpacing = 60; // catecoin: 1 minute blocks
...
{
    // FetchInputs can return false either CTCause we just haven't seen some inputs
    // (in which case the transaction should be stored as an orphan)
    // or CTCause the transaction is malformed (in which case the transaction should
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal catecoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // before the last blockchain checkpoint. This is safe CTCause block merkle hashes are
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // BIP16 didn't CTCome active until October 1 2012
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    {
        // the first block in the new chain that will cause it to CTCome the new best chain
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "catecoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3e8624cdd31c8ecf3e4589e8fd555d5b7d748716c102fee5dbe6d3d1a5bac6a4");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Love life and love catecoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1385524800; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5695628;

        if (fTestNet)
        {
            block.nTime    = 1385524800;
            block.nNonce   = 5695628;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x710629f7b8ccd2737a9f8a13e1ca83d6d9ff7348c71bd1ec26963b00abbc92a1"));
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        // Keep-alive ping. We send a nonce of zero CTCause we don't use it anywhere
        // right now.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // CTCause we're already processing them in order of dependency
